...
export default async function ListProjects() {
  let projects;
  try {
    projects = await db.timerProject.findMany();
  } catch (error) {
    ...
    return (
      <div key={project.id}>
        <div  className="grid grid-cols-7 gap-4">
          <p className="col-span-3">{project.project_name}</p>
          <p>{project.project_salary} $/h</p>
          <Link className="col-span-1" href={paths.timeTracker.showProject(project.id)}>
            <Button size="sm">View</Button>
          </Link>
          <NewTimerForm projectId={project.id} projectSalary={project.project_salary} redirectStatus={true}/>
