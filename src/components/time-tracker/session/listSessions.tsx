...
export default function ListSessions({ sessions }: ListSessionsProps) {
  ...
  sessions.forEach((session) => {
    totalMoney += session.money_earned;
  });
  ...
  sessions.sort((a, b) => b.session_date.getTime() - a.session_date.getTime());

  // Group sessions by month, week, and date
  const sessionsByMonth = sessions.reduce<SessionsByMonth>((acc, session) => {
  const month = format(session.session_date, 'MMMM', { locale: de });
  const weekOfMonth = getWeekOfMonth(session.session_date, { locale: de });
  const week = `${weekOfMonth}. Woche`;
    const date = session.session_date.toDateString();
  ...
  const renderedSessionsByMonth = Object.entries(sessionsByMonth).map(([month, weeks]) => {
    ...
    Object.values(weeks).forEach((days) => {
      Object.values(days).forEach((sessions) => {
        sessions.forEach((session) => (moneyPerMonth += session.money_earned));
      });
    });
      ...
      <Accordion key={month}>
        ...
        <AccordionDetails>
          {Object.entries(weeks).map(([week, days]) => {
            let moneyPerWeek = 0;
            Object.values(days).forEach((sessions) => {
              sessions.forEach((session) => (moneyPerWeek += session.money_earned));
            });
              ...
              <Accordion key={week}>
                ...
                <AccordionDetails>
                  {Object.entries(days).map(([date, sessions]) => {
                    let moneyPerDate = 0;
                    sessions.forEach((session) => (moneyPerDate += session.money_earned));
                          ...
                          {sessions.map((session) => (
                            <div key={session.id} className="pb-5">
                              <div className="grid grid-cols-5 gap-5">
                                <p className="text-center col-span-1">{session.money_earned} $</p>
                                <p className="text-center col-span-2">{session.time_spent} Minutes</p>
                                <EditSessionForm {...session} />
                                <DeleteSessionButton sessionId={session.id} projectId={session.project_id} />
                              </div>
                              <Divider />
                            </div>
                          ))}
