...
export default function ListSessions({ sessions }: ListSessionsProps) {
  ...
  const renderedSessionsByMonth = Object.entries(sessionsByMonth).map(
    ([month, weeks]) => {
      let moneyPerMonth = 0;
          ...
          <AccordionItem value={month}>
            ...
            <AccordionContent>
              {Object.entries(weeks).map(([week, days]) => {
                ...
                Object.values(days).forEach((sessions) => {
                  sessions.forEach(
                    (session) => (moneyPerWeek += session.moneyEarned)
                  );
                });
                    ...
                    <AccordionItem value={week}>
                      ...
                      <AccordionContent>
                        {Object.entries(days).map(([date, sessions]) => {
                          let moneyPerDate = 0;
                          sessions.forEach(
                            (session) => (moneyPerDate += session.moneyEarned)
                          );
